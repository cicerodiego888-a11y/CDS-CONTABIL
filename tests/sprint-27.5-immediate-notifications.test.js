...
test('1 forgot-password cria evento', async () => {
  const before = db.prepare(
    'SELECT COUNT(*) n FROM domain_events WHERE tenant_id=? AND event_type=? AND entity_id=?'
  ).get(ownerA.user.tenant_id, EVENT_TYPES.CLIENT_PASSWORD_RESET_REQUESTED, clientA1.user.id).n;
...
test('2 evento cria notification', async () => {
  ...
  ).get(ownerA.user.tenant_id, EVENT_TYPES.CLIENT_PASSWORD_RESET_REQUESTED, clientA1.user.id);
  const rows = db.prepare('SELECT * FROM notifications WHERE event_id=?').all(ev.id);
  assert.ok(rows.length >= 1);
  assert.ok(rows.every(n => n.type === EVENT_TYPES.CLIENT_PASSWORD_RESET_REQUESTED));
...
test('3 notification aparece no endpoint imediatamente e primeiro', async () => {
  const n = await req('GET', '/api/notificacoes?page=1&page_size=25', undefined, ownerA.token);
  assert.equal(n.status, 200);
  const hit = (n.data.items || []).find(
    x => x.type === EVENT_TYPES.CLIENT_PASSWORD_RESET_REQUESTED && x.entity_id === clientA1.user.id
  );
  assert.ok(hit);
  assert.equal((n.data.items || [])[0].id, hit.id);
  assert.equal(hit.title, 'Solicitação de redefinição de acesso');
...
test('4 OWNER e ACCOUNTANT recebem; CLIENT/STAFF não', async () => {
  ...
  assert.ok((owner.data.items || []).some(x => x.entity_id === clientA1.user.id));
  assert.ok((acc.data.items || []).some(x => x.entity_id === clientA1.user.id));
  assert.equal(
    (staff.data.items || []).filter(x => x.type === EVENT_TYPES.CLIENT_PASSWORD_RESET_REQUESTED).length,
    0
  );
  ...
  if (clientNotes.status === 200) {
    assert.equal(
      (clientNotes.data.items || []).filter(x => x.type === EVENT_TYPES.CLIENT_PASSWORD_RESET_REQUESTED).length,
      0
    );
  }
});
...
test('6 tenant isolation', async () => {
  ...
  assert.ok((nB.data.items || []).some(
    x => x.entity_id === clientB.user.id && x.type === EVENT_TYPES.CLIENT_PASSWORD_RESET_REQUESTED
  ));
});
...
test('7 company isolation no contexto da notification', async () => {
  ...
  const hit = (n.data.items || []).find(x => x.entity_id === clientA2.user.id);
  assert.ok(hit);
  assert.equal(hit.company_id, companyA2.id);
  assert.notEqual(hit.company_id, companyA1.id);
});

test('8 notification não duplica (dedupe unread)', async () => {
  const before = db.prepare(
    'SELECT COUNT(*) n FROM domain_events WHERE tenant_id=? AND event_type=? AND entity_id=?'
  ).get(ownerA.user.tenant_id, EVENT_TYPES.CLIENT_PASSWORD_RESET_REQUESTED, clientA1.user.id).n;
...
test('9 read/unread atualiza contagem', async () => {
  ...
  const hit = (before.data.items || []).find(
    x => x.type === EVENT_TYPES.CLIENT_PASSWORD_RESET_REQUESTED && x.entity_id === clientA1.user.id && !x.read_at
  );
  assert.ok(hit);
...
test('10 duas solicitações independentes', async () => {
  db.prepare(
  ...
  const unreadEntities = (n.data.items || [])
    .filter(x => x.type === EVENT_TYPES.CLIENT_PASSWORD_RESET_REQUESTED && !x.read_at)
    .map(x => x.entity_id);
