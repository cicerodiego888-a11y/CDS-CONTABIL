...
test('HTTP serve a implementação nova de Nova despesa',async()=>{
  ...
  const portalJs=await fetch(base+'/portal/portal.js?v=s03-2').then(r=>r.text());
  assert.match(portalJs,/Salvar despesa/);
  assert.match(portalJs,/Arraste e solte um arquivo aqui/);
  assert.doesNotMatch(portalJs.slice(portalJs.indexOf('async function transactionForm'),portalJs.indexOf('async function detail')),/<label>Categoria<\/label>/);
  assert.match(portalJs,/<option value="">Categoria<\/option>/);
  assert.match(portalJs,/Despesa registrada com sucesso\. Ela foi enviada para análise da contabilidade\./);
  ...
  const adminJs=await fetch(base+'/assets/app.js?v=s03-2').then(r=>r.text());
  assert.match(adminJs,/Salvar despesa/);
  assert.match(adminJs,/Arraste e solte um arquivo aqui/);
