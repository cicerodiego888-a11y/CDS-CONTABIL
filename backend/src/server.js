...
const {createEmailProviderFromEnv,createEmailProvider,fromEnv,diagnose,resolveName}=require('./email/provider');
...
function rateLogin(req,res,next){const max=Number(process.env.CDS_LOGIN_MAX)||(IS_PROD?8:200);const windowMs=15*60*1000,now=Date.now();const key=(req.ip||'')+'|'+String(req.body?.email||'').toLowerCase()+'|'+String(req.body?.tenant||req.body?.tenant_slug||'').toLowerCase();const cur=loginAttempts.get(key)||[];const fresh=cur.filter(t=>now-t<windowMs);if(fresh.length>=max)return res.status(429).json({error:'TOO_MANY_ATTEMPTS',message:'Credenciais inválidas.'});fresh.push(now);loginAttempts.set(key,fresh);next()}
const forgotAttempts=new Map();
function rateForgot(req,res,next){const max=Number(process.env.CDS_FORGOT_MAX)||(IS_PROD?8:200);const windowMs=15*60*1000,now=Date.now();const key=(req.ip||'')+'|forgot|'+String(req.body?.email||'').toLowerCase()+'|'+String(req.body?.tenant||req.body?.tenant_slug||'').toLowerCase();const cur=forgotAttempts.get(key)||[];const fresh=cur.filter(t=>now-t<windowMs);if(fresh.length>=max)return res.status(429).json({error:'TOO_MANY_ATTEMPTS',message:'Se os dados informados corresponderem a um acesso cadastrado, o escritório contábil será notificado para realizar a redefinição.'});fresh.push(now);forgotAttempts.set(key,fresh);next()}
...
const clientFileTypes={'.pdf':['application/pdf'],'.jpg':['image/jpeg','image/jpg'],'.jpeg':['image/jpeg'],'.png':['image/png']};
function validateClientFile(file){if(!file)return 'Envie um arquivo.';const ext=path.extname(file.originalname||'').toLowerCase();const allowed=clientFileTypes[ext];if(!allowed)return 'Formato não autorizado. Envie JPG, JPEG, PNG ou PDF.';if(file.size>CLIENT_UPLOAD_MAX)return 'O arquivo excede o tamanho máximo permitido.';const mime=String(file.mimetype||'').toLowerCase();if(mime&&!allowed.includes(mime)&&mime!=='application/octet-stream')return 'O tipo do arquivo não é permitido.';return null}
...
function appPublicUrl(){
  const fromEnv=String(process.env.CDS_EMAIL_APP_URL||process.env.PUBLIC_URL||'').trim();
...
function portfolioAggregates(req,companyId){
  ...
  const documents_received=one(`SELECT COUNT(*) n FROM documents WHERE tenant_id=? AND deleted_at IS NULL${cf}`,...p).n;
...
app.post('/api/auth/login',rateLogin,(req,res)=>{const email=String(req.body.email||'').trim();const password=String(req.body.password||'');const tenantKey=String(req.body.tenant||req.body.tenant_slug||req.body.office||'').trim().toLowerCase();let tenant=tenantKey?one('SELECT * FROM tenants WHERE lower(IFNULL(slug,\'\'))=? OR id=?',tenantKey,tenantKey):null;if(!tenant&&tenantKey==='demo')tenant=one("SELECT * FROM tenants WHERE lower(IFNULL(slug,''))='escritorio-demonstracao'");const u=tenant?one('SELECT * FROM users WHERE tenant_id=? AND lower(email)=lower(?)',tenant.id,email):null;let ok=false;try{ok=bcrypt.compareSync(password,u?.password_hash||DUMMY_HASH)}catch{ok=false}if(!tenant||!u||!u.active||!ok)return deny(res,401,'Credenciais inválidas.','INVALID_CREDENTIALS');if(u.role==='CLIENT'){const company=u.company_id?one('SELECT status FROM companies WHERE tenant_id=? AND id=?',u.tenant_id,u.company_id):null;if(!company||company.status!=='ACTIVE')return deny(res,401,'Esta empresa está bloqueada ou indisponível.','COMPANY_UNAVAILABLE')}exec('UPDATE users SET last_access_at=CURRENT_TIMESTAMP WHERE id=?',u.id);const profile=u.role==='CLIENT'?clientProfile({user:{sub:u.id}}):null;const fresh=one('SELECT * FROM users WHERE id=?',u.id);const token=issueToken(fresh,profile);audit({user:{tenant_id:u.tenant_id,sub:u.id},ip:req.ip},'LOGIN','USER',u.id,null,{role:u.role,result:'ok'});const tenantSlug=one('SELECT slug FROM tenants WHERE id=?',u.tenant_id)?.slug||null;if(config.AUTH_COOKIE)res.cookie('cds_session',token,{httpOnly:true,secure:IS_PROD,sameSite:'lax',path:'/',maxAge:12*60*60*1000});res.json({token,redirect:u.role==='CLIENT'?'/portal/':'/',user:{id:u.id,name:u.name,email:u.email,role:u.role,tenant_id:u.tenant_id,tenant_slug:tenantSlug,company_id:u.company_id,profile,permissions:u.role==='CLIENT'?clientPermissionList(u.id):null}})});
const FORGOT_GENERIC='Se os dados informados corresponderem a um acesso cadastrado, o escritório contábil será notificado para realizar a redefinição.';
app.post('/api/auth/forgot-password',rateForgot,(req,res)=>{
  ...
  if(!email||!tenantKey||!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email))return respond();
  ...
  const u=one("SELECT * FROM users WHERE tenant_id=? AND lower(email)=? AND role='CLIENT'",tenant.id,email);
  if(!u||!u.company_id)return respond();
  ...
  const existing=one(
    "SELECT id FROM notifications WHERE tenant_id=? AND company_id=? AND entity_type='client_user' AND entity_id=? AND type=? AND read_at IS NULL LIMIT 1",
    tenant.id,company.id,u.id,EVENT_TYPES.CLIENT_PASSWORD_RESET_REQUESTED
  );
  if(existing){
    auditSystem(tenant.id,u.id,'CLIENT_PASSWORD_RESET_REQUESTED','USER',u.id,{company_id:company.id,client_user_id:u.id,result:'deduped'});
    return respond();
  }
  emitEvent({
    tenantId:tenant.id,
  ...
  auditSystem(tenant.id,u.id,'CLIENT_PASSWORD_RESET_REQUESTED','USER',u.id,{company_id:company.id,client_user_id:u.id,result:'ok'});
  return respond();
});
...
app.get('/api/auth/me',auth,(req,res)=>{const user=one('SELECT id,name,email,role,tenant_id,company_id,active,created_at,last_access_at FROM users WHERE id=? AND tenant_id=?',req.user.sub,req.user.tenant_id);if(!user)return deny(res,401,'Sessão inválida.','INVALID_TOKEN');user.tenant_slug=one('SELECT slug FROM tenants WHERE id=?',user.tenant_id)?.slug||null;if(user.role==='CLIENT'){user.profile=clientProfile(req);user.permissions=clientPermissionList(user.id)}res.json(user)});
...
app.get('/api/empresas/:id/operacional',auth,role('OWNER','ACCOUNTANT','STAFF'),(req,res)=>{
  ...
  const documents=one('SELECT COUNT(*) n FROM documents WHERE tenant_id=? AND company_id=? AND deleted_at IS NULL',tenantId,companyId).n;
...
app.post('/api/client-users/:id/redefinir-acesso',auth,scope,role('OWNER','ACCOUNTANT'),async(req,res)=>{
  ...
  exec('UPDATE users SET password_hash=? WHERE id=? AND tenant_id=?',bcrypt.hashSync(crypto.randomBytes(32).toString('hex'),12),u.id,u.tenant_id);
...
app.post('/api/regras-contabeis',auth,role('OWNER','ACCOUNTANT','STAFF'),(req,res)=>{if(!req.body.name)return res.status(400).json({error:'NAME_REQUIRED'});const x=id();exec('INSERT INTO accounting_rules(id,tenant_id,company_id,name,priority,conditions_json,debit_account_id,credit_account_id,settlement_account_id,confidence) VALUES(?,?,?,?,?,?,?,?,?,?)',x,req.user.tenant_id,req.body.company_id||null,req.body.name,Number(req.body.priority||100),JSON.stringify(req.body.conditions||{}),req.body.debit_account_id||null,req.body.credit_account_id||null,req.body.settlement_account_id||null,Number(req.body.confidence??1));res.status(201).json(one('SELECT * FROM accounting_rules WHERE id=?',x))});
app.patch('/api/regras-contabeis/:id',auth,role('OWNER','ACCOUNTANT','STAFF'),(req,res)=>{const b=one('SELECT * FROM accounting_rules WHERE tenant_id=? AND id=?',req.user.tenant_id,req.params.id);if(!b)return res.status(404).json({error:'NOT_FOUND'});exec('UPDATE accounting_rules SET name=?,company_id=?,priority=?,conditions_json=?,debit_account_id=?,credit_account_id=?,settlement_account_id=?,confidence=?,active=? WHERE id=? AND tenant_id=?',req.body.name??b.name,req.body.company_id===undefined?b.company_id:req.body.company_id||null,req.body.priority??b.priority,JSON.stringify(req.body.conditions??JSON.parse(b.conditions_json||'{}')),req.body.debit_account_id??b.debit_account_id,req.body.credit_account_id??b.credit_account_id,req.body.settlement_account_id??b.settlement_account_id,req.body.confidence??b.confidence,req.body.active===false?0:1,b.id,req.user.tenant_id);res.json({ok:true})});
...
function storeClientDocument(req,file,notes){const err=validateClientFile(file);if(err){try{fs.unlinkSync(file.path)}catch{}throw Object.assign(new Error(err),{http:422});}const x=id(),companyId=clientCompanyId(req);const stored=documentStorage.ingestUpload(file,x);const origin=documentOwnership.originForActor(req.user,origens),source=documentOwnership.sourceForActor(req.user);exec('INSERT INTO documents(id,tenant_id,company_id,original_name,storage_path,mime_type,size_bytes,sha256,uploaded_by,notes,status,origin,source,encrypted,encryption_kid) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',x,req.user.tenant_id,companyId,file.originalname,stored.storage_path,file.mimetype,stored.size_bytes,stored.sha256,req.user.sub,notes||null,'PENDING_REVIEW',origin,source,stored.encrypted?1:0,stored.encryption_kid);emitFromReq(req,EVENT_TYPES.DOCUMENT_UPLOADED,{companyId,entityType:'document',entityId:x,payload:{original_name:file.originalname}});audit(req,'CLIENT_UPLOADED_DOCUMENT','DOCUMENT',x,null,{original_name:file.originalname,sha256:stored.sha256,source,origin,storage_path:stored.storage_path});return publicDocument(one('SELECT * FROM documents WHERE id=?',x),req.user.tenant_id,req)}
app.get('/api/client/documentos',auth,requireClient,requireClientCompany,(req,res)=>{const companyId=clientCompanyId(req),p=[req.user.tenant_id,companyId];let sql='SELECT * FROM documents WHERE tenant_id=? AND company_id=? AND deleted_at IS NULL';if(req.query.from){sql+=' AND date(created_at)>=date(?)';p.push(req.query.from)}if(req.query.to){sql+=' AND date(created_at)<=date(?)';p.push(req.query.to)}if(req.query.status){sql+=' AND status=?';p.push(req.query.status)}if(req.query.type==='pdf')sql+=" AND lower(IFNULL(mime_type,'')) LIKE '%pdf%'";if(req.query.type==='image')sql+=" AND lower(IFNULL(mime_type,'')) LIKE 'image/%'";sql+=' ORDER BY created_at DESC';let rows=qRows(sql,...p).map(d=>publicDocument(d,req.user.tenant_id,req)).filter(Boolean);if(req.query.linked==='1')rows=rows.filter(x=>x.linked);if(req.query.linked==='0')rows=rows.filter(x=>!x.linked);res.json(rows)});
app.post('/api/client/documentos',auth,requireClient,requireClientCompany,clientUpload.single('file'),(req,res)=>{try{if(!req.file)return deny(res,400,'Envie um arquivo.','FILE_REQUIRED');res.status(201).json(storeClientDocument(req,req.file,req.body.notes||req.body.observacao))}catch(e){if(req.file)try{fs.unlinkSync(req.file.path)}catch{}res.status(e.http||422).json({error:'UPLOAD_FAILED',message:e.message})}});
...
const exportService=mountExportRoutes(app,{db,id,auth,role,scope,deny,audit,companyOk,today,pageParams,paged,scopedCompanyWhere,exportDir:EXPORT});
...
if(documentIntelligence&&documentIntelligence.service&&documentIntelligence.service.setVisualAi){
  documentIntelligence.service.setVisualAi({
    available(tenantId){
      return !!(aiControlService&&aiControlService.availability(tenantId).available&&
        accountingAIService&&accountingAIService.providerInfo().configured);
    },
...
app.post('/api/invitations/:token/accept',(req,res)=>{
  ...
  const isReset=purpose==='PASSWORD_RESET';
  db.transaction(()=>{
    exec('UPDATE users SET name=?,password_hash=?,active=1 WHERE id=? AND tenant_id=? AND company_id=?',name,bcrypt.hashSync(password,12),i.user_id,i.tenant_id,i.company_id);
  ...
  res.json({message:isReset?'Senha criada com sucesso.':'Conta ativada com sucesso',token,redirect:'/portal/',user:{id:i.user_id,role:'CLIENT',profile,company_id:i.company_id,tenant_id:i.tenant_id}});
});
