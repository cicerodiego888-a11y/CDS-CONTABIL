...
function detectAuthCookieMode(){if(authModeReady)return authModeReady;authModeReady=fetch('/api/health',{credentials:'include',cache:'no-store'}).then(r=>r.json()).then(h=>{authCookieMode=!!(h&&h.auth_cookie);if(authCookieMode){try{localStorage.removeItem('ccc_office_token');localStorage.removeItem('ccc_token')}catch{}if(state.token&&!/^eyJ/.test(String(state.token))){/* keep */}else if(authCookieMode&&state.token){/* may clear after session confirm */} }return authCookieMode}).catch(()=>{authCookieMode=false;return false});return authModeReady}
function setOfficeToken(token){try{if(authCookieMode){localStorage.removeItem('ccc_office_token');localStorage.removeItem('ccc_token');state.token=token||null;return}if(token){state.token=token;localStorage.setItem('ccc_office_token',token);localStorage.removeItem('ccc_token')}else{state.token=null;localStorage.removeItem('ccc_office_token')}}catch{state.token=token||null}}
function clearSession(msg){if(window.__cdsOfficeEs){try{window.__cdsOfficeEs.close()}catch{}window.__cdsOfficeEs=null}if(state.brandingLogoSrc){try{URL.revokeObjectURL(state.brandingLogoSrc)}catch{}}state.branding=null;state.brandingLogoSrc=null;try{localStorage.removeItem('ccc_office_token');localStorage.removeItem('ccc_token')}catch{}/* Sprint 28.3: não remove ccc_client_token */state.token=null;state.user=null;state.selectedCompany=null;fetch('/api/auth/logout',{method:'POST',credentials:'include'}).catch(()=>{});toast(msg||'Sua sessão expirou. Entre novamente para continuar.','warning');login()}
...
function authHeaders(path){const h={};if(state.token&&state.token!=='cookie')h.Authorization='Bearer '+state.token;if(state.selectedCompany?.id&&!isTenantScopedPath(path||''))h['X-Company-Id']=state.selectedCompany.id;return h}
...
function entryDocumentHtml(e){const d=e&&e.document;if(!d)return '<p class="muted">Nenhum documento vinculado.</p>';const kind=entryDocKind(d);const preview=kind==='other'?`<div class="entry-doc-preview entry-doc-preview--empty" id="entryDocPreview"><p class="muted">Pré-visualização indisponível para este tipo de arquivo.</p></div>`:`<div class="entry-doc-preview" id="entryDocPreview" data-kind="${kind}" data-zoom="1" aria-busy="true"><div class="entry-doc-preview-loading muted">Carregando pré-visualização…</div></div>`;const zoom=kind==='other'?'':`<div class="entry-doc-zoom" role="group" aria-label="Zoom do documento"><button type="button" class="btn secondary" id="entryDocZoomOut" aria-label="Diminuir zoom">−</button><span class="entry-doc-zoom-label" id="entryDocZoomLabel">100%</span><button type="button" class="btn secondary" id="entryDocZoomIn" aria-label="Aumentar zoom">+</button></div>`;return `${preview}<p class="entry-doc-name"><b>${esc(d.original_name)}</b></p><div class="row-actions">${zoom}<button type="button" class="btn secondary" id="viewEntryDoc">Visualizar</button></div>`}
function applyEntryDocZoom(){const box=$('#entryDocPreview');if(!box)return;const z=Math.min(3,Math.max(0.5,Number(box.dataset.zoom||1)||1));box.dataset.zoom=String(z);const target=box.querySelector('img,iframe');if(target){target.style.transform='scale('+z+')';target.style.transformOrigin='center top'}const label=$('#entryDocZoomLabel');if(label)label.textContent=Math.round(z*100)+'%';const out=$('#entryDocZoomOut'),inn=$('#entryDocZoomIn');if(out)out.disabled=z<=0.5;if(inn)inn.disabled=z>=3}
function bindEntryDoc(e){const d=e&&e.document;const b=$('#viewEntryDoc');if(b&&d)b.onclick=()=>viewOfficeDocument(d.id,d.original_name,d.mime_type||'',d.size_bytes||0);const box=$('#entryDocPreview');const zoomOut=$('#entryDocZoomOut'),zoomIn=$('#entryDocZoomIn');if(zoomOut)zoomOut.onclick=()=>{if(!box)return;box.dataset.zoom=String(Math.max(0.5,(Number(box.dataset.zoom||1)||1)-0.25));applyEntryDocZoom()};if(zoomIn)zoomIn.onclick=()=>{if(!box)return;box.dataset.zoom=String(Math.min(3,(Number(box.dataset.zoom||1)||1)+0.25));applyEntryDocZoom()};if(!box||!d)return;const kind=box.dataset.kind||entryDocKind(d);if(kind==='other')return;revokeEntryDocPreview();(async()=>{try{const r=await fetch('/api/documentos/'+d.id+'/view',{headers:authHeaders('/documentos/'+d.id+'/view'),cache:'no-store'});if(!r.ok)throw Error('Falha ao carregar documento');const blob=await r.blob();const url=URL.createObjectURL(blob);state.entryDocObjectUrl=url;if(!$('#entryDocPreview')){URL.revokeObjectURL(url);state.entryDocObjectUrl=null;return}if(kind==='image')box.innerHTML=`<img src="${url}" alt="Pré-visualização de ${esc(d.original_name)}">`;else box.innerHTML=`<iframe title="Pré-visualização do PDF" src="${url}#toolbar=0"></iframe>`;box.setAttribute('aria-busy','false');box.dataset.zoom='1';applyEntryDocZoom()}catch{if($('#entryDocPreview')){$('#entryDocPreview').innerHTML='<p class="muted">Não foi possível carregar a pré-visualização.</p>';$('#entryDocPreview').setAttribute('aria-busy','false')}}})()}
...
function entrySupplierName(e){if(!e)return '';const fromMove=e.movement&&e.movement.supplier_name;if(fromMove)return String(fromMove).trim();const pipe=e.pipeline&&e.pipeline.fields&&e.pipeline.fields.supplier_name;const fromPipe=pipe&&(pipe.value!=null?pipe.value:pipe);if(fromPipe)return String(fromPipe).trim();if(e.supplier_name)return String(e.supplier_name).trim();return ''}
function movementFactsHtml(e){const m=e.movement||{};const supplier=entrySupplierName(e);return `<p><b>${esc(e.description)}</b></p><p><small class="muted">Fornecedor</small><br><b>${esc(supplier||'Não identificado')}</b></p><p class="muted">${esc(e.company_name||'-')} · ${esc(e.occurred_on||'-')} · ${money(m.amount_cents||e.debit||e.source_cents||0)}</p><p class="muted">Pagamento: ${esc(originLabel(m.method||'-'))} · Banco: ${esc(m.bank_name||'Não informado')} · Origem: ${esc(originLabel(e.origin||e.source_type))}</p>`}
...
async function render(){await detectAuthCookieMode();if(!state.token&&authCookieMode){try{state.user=await api('/auth/me');state.token=state.token||'cookie'}catch(e){return login()}}if(!state.token)return login();try{state.user=state.user||await api('/auth/me');if(state.user.requires_pin_setup)return showOfficePinGate();if(state.user.role==='CLIENT'){window.location.href='/portal/';return}try{if(authCookieMode){localStorage.removeItem('ccc_office_token');localStorage.removeItem('ccc_token')}else if(state.token&&!localStorage.getItem('ccc_office_token')){localStorage.setItem('ccc_office_token',state.token);if(localStorage.getItem('ccc_token')===state.token)localStorage.removeItem('ccc_token')}}catch{}await restoreCompanyFromUrl();await loadBase();await loadSidebarCounters()}catch(e){if(e&&e.status===401)return;if(!state.user){toast(e.message||'Não foi possível concluir a operação.','error');return}toast(e.message||'Não foi possível concluir a operação.','error')}
...
async function reviewApproval(id){
  ...
  const rightEntry=`<div class="panel approval-entry-pane"><h3>Lançamento sugerido</h3>
    <div class="form-grid" style="gap:8px">
      <div><small class="muted">Fornecedor</small><div><b>${esc(entrySupplierName(e)||'-')}</b></div></div>
