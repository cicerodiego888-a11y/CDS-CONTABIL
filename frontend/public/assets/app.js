...
function login(){
  ...
  fetch('/api/health').then(r=>r.json()).then(h=>{
    ...
    if(guessDemo){
      if(t&&t.value===demoTenant)t.value='';
...
async function loadNotifications(since){const q='?page=1&page_size=25'+(since?'&since='+encodeURIComponent(since):'');const data=await api('/notificacoes'+q);data.items=dedupeNotifications(data.items||[]);state.notifications=data;state.unread=data.unread||0;return data}
function paintNotifList(items){const box=$('#notifList');if(!box)return;const list=dedupeNotifications(items);if(!list.length){box.innerHTML=emptyState('Nenhuma notificação','Você está em dia. Novos avisos aparecerão aqui.');return}box.innerHTML=list.map(n=>`<button type="button" class="notif-item ${n.read_at?'read':'unread'}" data-id="${esc(n.id)}"><b>${esc(n.title)}</b><span>${esc(n.message)}</span>${n.context?`<small>${esc(n.context)}</small>`:''}<small>${esc(timeAgo(n.created_at))}${n.read_at?' · lida':' · não lida'}</small>${n.occurrence_id?'<small><b>Abrir processo →</b></small>':''}</button>`).join('');box.querySelectorAll('.notif-item').forEach(btn=>btn.onclick=()=>openNotification(btn.dataset.id))}
...
async function refreshNotifBadge(){if(!state.token||!state.user||state.user.role==='CLIENT')return;try{const data=await loadNotifications();applyNotifBadge(data);const panel=$('#notifPanel');if(panel&&!panel.hidden)paintNotifList(data.items||[])}catch{/* inbox poll must never take down the portal */}}
...
async function drawNotifList(){const box=$('#notifList');if(!box)return;box.innerHTML='<div class="empty">Carregando...</div>';try{const data=await loadNotifications();applyNotifBadge(data);paintNotifList(data.items||[])}catch(e){box.innerHTML=`<div class="empty">Erro: ${esc(e.message)}</div>`}}
async function openNotification(id){const n=(state.notifications?.items||[]).find(x=>x.id===id);try{await api('/notificacoes/'+id+'/lida',{method:'POST',body:'{}'})}catch{}
  ...
  if(n.type==='CLIENT_PASSWORD_RESET_REQUESTED'||n.entity_type==='client_user'){
    const userId=n.target_user_id||n.entity_id||n.reference_id;
    if(!n.company_id||!userId){toast('Não foi possível abrir o usuário desta solicitação.','warning');return refreshNotifBadge()}
    try{
...
function extractionStatusBadge(status){const map={PENDING:['pending','PENDENTE'],PROCESSING:['need','PROCESSANDO'],EXTRACTED:['approved','EXTRAÍDO'],FAILED:['rejected','FALHOU'],REVIEWED:['approved','REVISADO']};const x=map[status];return x?`<span class="badge ${x[0]}">${x[1]}</span>`:'<span class="muted">Não analisado</span>'}
...
async function documentExtractionModal(id,initial){let x=initial;try{x=x||await api('/documentos/'+id+'/extracao')}catch(e){if(e.status===404)return requestDocumentExtraction(id);return toast(e.message)}
  ...
  modal(`<form id="extractionForm">${modalHead('Inteligência Documental',esc(x.document_name||'Documento'))}<div class="modal-body"><div style="display:flex;justify-content:space-between;gap:12px;align-items:center;margin-bottom:14px"><div>${extractionStatusBadge(x.status)} <span class="muted">${esc(x.extraction_method||'')}</span></div><button type="button" class="btn secondary" id="viewExtractionDoc">Visualizar original</button></div>${failed?`<div class="panel"><b>Extração indisponível</b><p>${esc(x.error_message||'Não foi possível extrair este documento.')}</p></div>`:`<div class="form-grid">${fieldHtml}</div>`}${x.extracted_text?`<details style="margin-top:14px"><summary>Texto original extraído</summary><pre style="white-space:pre-wrap;max-height:220px;overflow:auto">${esc(x.extracted_text)}</pre></details>`:''}<p class="muted">A confirmação não cria lançamento contábil. Os dados ficam prontos para a próxima etapa de classificação.</p></div>${modalFoot(`<button type="button" class="btn secondary" onclick="closeModal()">Fechar</button>${failed||ready?'<button type="button" class="btn secondary" id="reprocessExtraction">Reprocessar</button>':''}${ready?'<button type="button" class="btn secondary" id="saveExtraction">Salvar correções</button>':''}${reviewed?'<button type="button" class="btn" id="requestAccountingAI">Classificação inteligente</button>':'<button class="btn" id="confirmExtraction">Confirmar dados</button>'}`)}</form>`,'lg');
...
async function processDetailPage(c,processId){
  ...
  c.innerHTML=head(esc(p.name),'Empresa: '+esc(p.company_trade_name||p.company_name||'-')+' · Responsável: '+esc(p.responsible_name||'-'),'<button class="btn secondary" id="back">← Voltar</button><button class="btn secondary" id="toggleStatus">'+(p.status==='ATIVO'?'Desativar':'Ativar')+'</button><button class="btn" id="saveMeta">Salvar processo</button>')+
...
async function aiSettingsPage(c){
  ...
  }catch(err){c.innerHTML=head('Inteligência Artificial','Controle de uso da IA por escritório.')+`<div class="panel">${emptyState('Não foi possível carregar',esc(err.message))}</div>`;return}
