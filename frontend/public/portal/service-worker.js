/* Sprint 28.3 — Service Worker do Portal do Cliente (escopo /portal/) */
...
function buildOptions(data) {
  ...
  const url = String((data.data && data.data.url) || data.url || '/portal/');
  ...
  const options = {
    ...
    tag: data.tag || (data.request_id ? ('request:' + data.request_id) : (entityId ? (String(type).toLowerCase() + ':' + entityId) : ('cds-portal-' + Date.now()))),
    renotify: true,
...
async function openTarget(targetUrl, data) {
  ...
  for (const client of all) {
    try {
      ...
      if (u.origin === self.location.origin && u.pathname.indexOf('/portal') === 0) {
        client.postMessage({
          type: 'CDS_PUSH_OPEN',
          url: targetUrl,
  ...
  if (self.clients.openWindow) {
    let openUrl = targetUrl || '/portal/';
    if (openUrl.charAt(0) === '/' && openUrl.indexOf('/portal') !== 0) {
      openUrl = '/portal/' + (openUrl === '/' ? '' : openUrl.replace(/^\//, ''));
    }
